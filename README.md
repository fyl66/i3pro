...
| **工程化** | 150 项单测全绿（真实数据回归 + HTTP 端到端 + 无头驱动前端 255 个断言点），零第三方运行期依赖；改这个仓库的硬性规则见 [AGENTS.md](AGENTS.md) |
...
node tools\smoke_viewer.js out\demo.html     # 无头驱动前端：缩放/光标/分组/信标编辑/数学通道/区段/报表等 255 个断言点
```
