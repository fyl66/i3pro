...
| **工程化** | 232 项单测全绿（真实数据回归 + HTTP 端到端 + 无头驱动前端 402 个断言点），另有 **真浏览器真鼠标验收 51 项**（`tools\verify_clicks.py`，Edge 的 DevTools 协议发真事件，跑在两份金标准上；有跳点的 `耐久正赛` 上 53 项），零第三方运行期依赖；改这个仓库的硬性规则见 [AGENTS.md](AGENTS.md) |
...
```powershell
python -m unittest discover -s tests -v      # 232 项，无数据文件时自动 skip
