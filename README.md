...
| **工作表 / 组件** | 像 i2 Pro 一样自由组合：添加 / 删除 / 上下移动 / 拖拽改高度，半宽组件自动并排；5 套预设（分析 / 双圈对比 / 动力 / 底盘 / 车手）；每个图有自己的通道列表；**整张工作表能编码进分享链接** |
| **图表抬头** | i2 Pro 的 Measurements 列：`通道名 \| 光标值 \| Min \| Max \| Avg`，光标值随鼠标实时跳动，不用低头看左栏 |
| **缩放不丢细节（serve 模式）** | 快照是预先抽稀的；`serve` 模式每次缩放按可见区间重新取样：1 秒窗口返回 100 个原始样本，单次请求 3 ms |
| **工程化** | 29 项单测全绿（真实数据回归 + HTTP 端到端 + 无头驱动前端 16 组交互断言），零第三方运行期依赖；改这个仓库的硬性规则见 [AGENTS.md](AGENTS.md) |
...
| 工作表 | 组件标题栏 | `↑` `↓` 移动 · `✕` 移除 · 右下角拖拽改高度 · 左上下拉选类型后「＋ 添加组件」 |
