...
| **工作表 / 组件** | 12 列网格，**拖标题栏移动、拖右下角同时改宽高、边缘吸附到相邻组件**；组件类型：时间/距离图 · 散点图 · 赛道轨迹 · **直方图**（柱状/折线/文字、按第三通道着色、门槛可写通道名或数学表达式）· **仪表**（数值/列表/条形/表盘/方向盘）· 时间差 Δ · 状态与故障 · 时间报告 · 通道报告；每个图有自己的通道列表；**整张工作表连同位置尺寸能编码进分享链接** |
| **工作表就是一个文件** | 顶上那排按钮 = 仓库里 `worksheets/*.json`，一文件一套（随版本发布的是 分析 / 对比 / 动力 / 底盘 / 车手 / 仪表台 / 报表）：加一套不用改代码，能 diff、能发给队友、换台机器就有，格式见 [`worksheets/README.md`](worksheets/README.md)。组件靠 `pick` **按提示词挑本场次的通道**（找不到就留空，不会乱指一条）；文件读坏了、或写了这一版不认识的组件类型，那一个被跳过并**在工作表那一栏说清是哪个文件、下一步做什么**。布局记忆**按场次**分（`i3pro.worksheet.v2:<场次>`），不再带着上一场的布局串场 |
...
| **工程化** | 300 项单测全绿（真实数据回归 + HTTP 端到端 + 无头驱动前端 468 个断言点），另有 **真浏览器真鼠标验收 75 项**（`tools\verify_clicks.py`，Edge 的 DevTools 协议发真事件，跑在两份金标准上），零第三方运行期依赖；改这个仓库的硬性规则见 [AGENTS.md](AGENTS.md) |
...
├─ sidecar.py       六种侧车文件（信标/区段/GPS/注释/数学通道/CSV 列映射）的唯一读写
...
```powershell
python -m unittest discover -s tests -v      # 300 项，无数据文件时自动 skip
python tools\verify_ld_vs_csv.py             # 与 i2 Pro CSV 逐通道对照
node tools\smoke_viewer.js out\demo.html     # 无头驱动前端：缩放/光标/分组/信标编辑/数学通道/区段/报表/直方图/频谱/注释/GPS 校正/导出/工作表等 468 个断言点
python tools\verify_clicks.py                # 真 Edge 发真鼠标/键盘：75 项交互验收（需要 Edge + 金标准数据）
```
