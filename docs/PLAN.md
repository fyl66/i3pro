...
单测（真实数据回归 + HTTP 端到端 + 无头 JS 冒烟）从当时的 21 项起一路加上来，现在 **215 项**
（`python -m unittest discover -s tests`）、零第三方运行期依赖、git 仓库。

**验收从"三条"变"四条"**：无头冒烟（`tools/smoke_viewer.js`，402 个断言点）跑在
...
| 人员流动导致烂尾 | 高/高 | 纯函数算法层 + 232 项单测 + 中文文档 + 零依赖 | 已闭环 |
...
9. Channel Bits、GPS 校正（ticket #14）、外观配色方案
10. 三电极值报表（电机温度 / 母线功率 / 能耗）
11. **架构评审的六张票（#16–#21）** —— 正在进行。评审量出来的事实是：最近 7 张功能票
    每张都改 7–10 个文件，交集恒为 `render.py` + `server.py` + `viewer.html` + 两个断言
    工具；于是三处最深的接缝各开了一张票——侧车文件读写（#16）、**组件类型注册表**
    （#17 → #19 图表类 → #20 表格与仪表类 → #21 取数收成一条路径）、派生通道接缝（#18）。
    **注册表的第一批（#17）已完成，验收 A40**：三类最简单的显示形式搬进声明表，
    `type === "…"` 从 88 处降到 73 处，行为零变化（老的分享链接也照旧解得出来）。
