...
0. **CSV / 任意 CSV 导入**（第一轮 Q1 已确认要做）—— `.ld/.ldx` 之外，
   支持 i2 Pro 导出的 CSV（`motec_csv.py` 已能读）以及"任意 CSV + 通道映射"，
   覆盖"别的队/别的设备给我一张表"的场景。
1. **数学通道编辑器** —— 它是 gating 通道、派生量、报表统计、告警条件的地基，
   没它做直方图/散点 gating 等于做一半。
2. **Track Editor / 区段定义** —— 有了区段才能做「双击区段名放大到该弯/直道」（i2 Pro 的
   `To Zoom to a Range: double-click on the range band`）、区段报表、Eclectic 理论最快圈
3. Histogram / Suspension Histogram / FFT（按 M6 的组件系统直接加类型即可）
4. Channel Report / Time Report（表格化统计 + Eclectic）
5. Gauges（表盘 / 条形 / 方向盘 / 状态灯）+ 动画播放
6. Alarms 告警（对三电安全预警有用；第一轮说不做，第二轮改成待办）
7. 手动打点切圈（GPS 失效时兜底）
8. 注释 / Notes、Channel Bits、GPS 校正、外观配色方案
