...
单测（真实数据回归 + HTTP 端到端 + 无头 JS 冒烟）从当时的 21 项起一路加上来，现在 **161 项**
（`python -m unittest discover -s tests`）、零第三方运行期依赖、git 仓库。

**验收从"三条"变"四条"**：无头冒烟（`tools/smoke_viewer.js`，281 个断言点）跑在
...
| 人员流动导致烂尾 | 高/高 | 纯函数算法层 + 161 项单测 + 中文文档 + 零依赖 | 已闭环 |
...
5. ~~**Histogram**~~ —— **完成**（ticket #9，验收 A34）：柱状 / 折线 / 文字三种画法、
   ...
   窗口现问 `/histogram`，快照内嵌整场 + 每条完整圈的那几份。剩下的 **Suspension
   Histogram** 与 **FFT**（ticket #10）按同一套组件系统接着加。
